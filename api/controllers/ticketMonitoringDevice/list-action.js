module.exports = {
    ...
    friendlyName: 'List',
	description: 'List TicketMonitoringDevice',
    inputs: 
	{   
        filters: //
        {
    ...
    fn: async function (inputs, exits) 
	{
      try {
        ...
        if(inputs.filters){
            ...
            filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: TicketMonitoringDevice}); //procesa los filtros secundarios

        }
        
        filters['ticketMonitoringDeviceActive'] = true;
        
        var records = await TicketMonitoringDevice.count
        ({
            where: filters,
        });
        ...
        if(records  > 0  )
        {
            ...
            if(inputs.page <= pages && inputs.page > 0 )
            {
                ...
                var list = await TicketMonitoringDevice.find({
                    where: filters, 
                    limit: inputs.limit,
                    skip: valueSkip,
                    sort : sort
