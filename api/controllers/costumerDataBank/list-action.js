...
module.exports = {
    ...
    friendlyName: 'List',
	description: 'List CostumerDataBank',
    inputs: 
	{    
        //aqui va!!!
        filters: //
    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            filters['costumerDataBankActive'] = true;


            var records = await CostumerDataBank.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await CostumerDataBank.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort :JSON.parse(inputs.sort)
                    }).populate('costumerDataBankDataBankId');
