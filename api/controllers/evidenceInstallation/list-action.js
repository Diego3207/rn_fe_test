...
module.exports = {
	...
	description: 'List filter evidence of installation',
    inputs: 
	{    
        filters: //
        {
    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            filters['evidenceInstallationActive'] = true;
            var records = await EvidenceInstallation.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await EvidenceInstallation.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
