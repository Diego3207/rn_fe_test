...
module.exports = {
    ...
    friendlyName: 'List ',
	description: 'List  TrackerInstallations.',
    inputs: 
	{    
        filters: //
        {
    ...
    fn: async function (inputs, exits) 
	{
		...
		try 
		{
            ...
            if(inputs.filters){
                ...
                filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: TrackerInstallation}); //procesa los filtros secundarios

            }
			
            filters['trackerInstallationActive'] = true;
            
			var records = await TrackerInstallation.count
			({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await TrackerInstallation.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort : sort
