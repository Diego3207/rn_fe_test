    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            if(inputs.filters){
                ...
                filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: TicketCoordination}); //procesa los filtros secundarios

            }

            filters['ticketCoordinationActive'] = true;

            var records = await TicketCoordination.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await TicketCoordination.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort : sort
