...
module.exports = {
    ...
    friendlyName: 'List',
	description: 'List categories of dependencies',
    inputs: 
	{    
        //aqui va!!!
        filters: //
    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            if(inputs.filters){
                ...
                filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: DependencyCategory}); //procesa los filtros secundarios

            }

            filters['dependencyCatergoryActive'] = true;

            var records = await DependencyCategory.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await DependencyCategory.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort : sort
