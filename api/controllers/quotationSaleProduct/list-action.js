module.exports = {
    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            filters['quotationSaleProductActive'] = true;
            var records = await QuotationSaleProduct.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await QuotationSaleProduct.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort :JSON.parse(inputs.sort)})
                        .populate('quotationSaleProductProductId');
