    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            if(inputs.filters){
                ...
                filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: Dependency}); //procesa los filtros secundarios

            }

            filters['dependencyActive'] = true;

            var records = await Dependency.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await Dependency.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort : sort
