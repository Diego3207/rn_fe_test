    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            if(inputs.filters){
                ...
                filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: Tracker}); //procesa los filtros secundarios
            }
            filters['trackerActive'] = true;


            var records = await Tracker.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await Tracker.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort : sort
                    }).populate('trackerSimCardId');

                    if(list){
                        for (var i = 0; i < list.length; i++) 
						{
                            var iccid =  await Supply.findOne({ id: list[i]['trackerSimCardId'].simCardSupplyId , supplyActive: true });
                            list[i]['trackerSimCardId'].simCardIccid = iccid['supplyKey'];
                        }
