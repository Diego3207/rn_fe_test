...
module.exports = {
    ...
    friendlyName: 'List',
	description: 'List contact for phone of dependency',
    inputs: 
	{    
        filters: //
        {
    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            if(inputs.filters){
                ...
                filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: DependencyPhoneContact}); //procesa los filtros secundarios

            }

            filters['dependencyPhoneContactActive'] = true;

            var records = await DependencyPhoneContact.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await DependencyPhoneContact.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort : sort
