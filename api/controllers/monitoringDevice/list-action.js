module.exports = {
    ...
    friendlyName: 'List',
	description: 'List MonitoringDevice',
    inputs: 
	{   
        filters: //
        {
    ...
    fn: async function (inputs, exits) 
	{
      try {
        ...
        if(inputs.filters){
            ...
            filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: MonitoringDevice}); //procesa los filtros secundarios

        }
        
        filters['monitoringDeviceActive'] = true;
        
        var records = await MonitoringDevice.count
        ({
            where: filters,
        });
        ...
        if(records  > 0  )
        {
            ...
            if(inputs.page <= pages && inputs.page > 0 )
            {
                ...
                var list = await MonitoringDevice.find({
                    where: filters, 
                    limit: inputs.limit,
                    skip: valueSkip,
                    sort : sort
                }).populate('monitoringDeviceCostumerId');
