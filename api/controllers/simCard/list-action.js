    ...
    fn: async function (inputs, exits) 
	{
		...
		try 
		{
            ...
            if(inputs.filters){
                ...
                filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: SimCard}); //procesa los filtros secundarios
            }
			
            filters['simCardActive'] = true;
            
			var records = await SimCard.count
			({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await SimCard.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort : sort
                    }).populate('simCardSupplyId'); 
