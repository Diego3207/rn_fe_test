...
module.exports = {
    ...
    friendlyName: 'List',
	description: 'List provider product',
    inputs: 
	{    
        filters: //
        {
    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            filters['providerProductActive'] = true;
            var records = await ProviderProduct.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await ProviderProduct.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort :JSON.parse(inputs.sort)
                    }).populate('providerProductProductId');
