    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            if(inputs.filters){
                ...
                filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: IncidenceInvolvedDevice}); //procesa los filtros secundarios

            }

            filters['incidenceEvidenceActive'] = true;

            var records = await IncidenceInvolvedDevice.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await IncidenceInvolvedDevice.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort : sort
