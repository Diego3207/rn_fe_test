    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            if(inputs.filters){
                ...
                filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: SaleOrder}); //procesa los filtros secundarios

            }
            filters['saleOrderActive'] = true;


            var records = await SaleOrder.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await SaleOrder.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
                        sort : sort
                    }).populate('saleOrderQuotationSaleId')
                        .populate('saleOrderCfdiId')
                        .populate('saleOrderPayWayId')
                        .populate('saleOrderPayMethodId')
                        .populate('saleOrderTransmitter')
