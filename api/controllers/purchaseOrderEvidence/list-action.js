...
module.exports = {
	...
	description: 'List filter purchase order Evidence',
    inputs: 
	{    
        filters: //
        {
    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            filters['purchaseOrderEvidenceActive'] = true;
            var records = await PurchaseOrderEvidence.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    var list = await PurchaseOrderEvidence.find({
                        where: filters, 
                        limit: inputs.limit,
                        skip: valueSkip,
