    ...
    fn: async function (inputs, exits) 
	{
        try {
            ...
            if(inputs.filters){
                ...
                filters = await sails.helpers.processSecondaryFilter.with({filters: filters, mainModel: Product}); //procesa los filtros secundarios

            }
            filters['productActive'] = true;

            var records = await Product.count({
                where: filters,
            });
            ...
            if(records  > 0  )
            {
                ...
                if(inputs.page <= pages && inputs.page > 0 )
                {
                    ...
                    {
                        var proFile = await ProductFile.find({ productFileProductId: list[i].id, productFileActive: true });
                        list[i].productFile = proFile;
                    } 
