...
export class ListPackageComponent implements OnInit, OnDestroy {
    ...
    constructor(private miscService:MiscService,private cdref:ChangeDetectorRef ,private packageService: PackageService, private messageService: MessageService, private confirmationService: ConfirmationService)
	{
    ...
    list(){
        this.packageService.getAll(this.limit,this.page,this.sort)
        .subscribe((data: any)=>{
            if(data != null)
            {
                this.packages = data['object']['records'];
    ...
    filter(texto: any){
        this.packageService.getFilter(texto,this.limit, this.page,this.sort) // le sumo +1 ya que en sails le resto uno a la pagina (en sails quitare ese -1 )
            .subscribe((data: any)=>{
                if(data != null){
                    this.packages = data['object']['records'];
                    this.totalRows = data['object']['totalRecords'];
    ...
    confirmDelete(id:number) {
        this.packageService.disable(id).subscribe((data: any)=>{
            this.list();
    ...
    confirmDeleteSelected() {
		...
		{
            const ptt = this.packageService.disable(this.selectedElements[i].id).pipe
			(
				catchError((error) =>
				{
