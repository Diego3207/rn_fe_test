...
export class ListLocationComponent implements OnInit, OnDestroy {
    ...
    list()
    {
        this.locationService.getAll(this.limit,this.page,this.sort)
        .subscribe((data: any)=>{
            if(data != null)
            {
                this.locations = data['object']['records'];
    ...
    filter(texto: any)
    {
        this.locationService.getFilter(texto,this.limit, this.page,this.sort) // le sumo +1 ya que en sails le resto uno a la pagina (en sails quitare ese -1 )
        .subscribe((data: any)=>{
            if(data != null){
                this.locations = data['object']['records'];
                this.totalRows = data['object']['totalRecords'];
    ...
    delete(deleteType:number, object : Location) {
    ...
    confirmDelete(id:number) {
        this.locationService.disable(id).subscribe((data: any)=>{
            this.list();
    ...
    confirmDeleteSelected() {
		...
		{
            const ptt = this.locationService.disable(this.selectedLocations[i].id).pipe
			(
				catchError((error) =>
				{
