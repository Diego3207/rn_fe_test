...
@Component({
    templateUrl: './list.trackerInstallationReview.component.html'
})
export class ListTrackerInstallationReviewComponent implements OnInit, OnDestroy {
    @ViewChild('urlMap') urlMap!: ElementRef;
    coordenates:string;
    selectedTrackerInstallationReviews:  TrackerInstallationReview[] = [];
    confirmDisplaySelected: boolean = false;
    confirmDisplay: boolean = false;
	content:string;
    trackerInstallationReviews:  TrackerInstallationReview[];
    trackerInstallationReview:  TrackerInstallationReview;
    totalRows: number = 0;
    limit:number = 10 ;
    page:number  ;
    sort:string = '';
    ...
    list()
    {
        this.trackerInstallationReviewService.getAll(this.limit,this.page,this.sort)
        .subscribe((data: any)=>{
            if(data != null)
            {
                this.trackerInstallationReviews = data['object']['records'];
                this.totalRows = data['object']['totalRecords'];
               // console.log(this.trackerUninstalls);

            }else{
                this.trackerInstallationReviews  = [];
                this.totalRows = 0;
    ...
    filter(texto: any)
    {
        this.trackerInstallationReviewService.getFilter(texto,this.limit, this.page,this.sort) // le sumo +1 ya que en sails le resto uno a la pagina (en sails quitare ese -1 )
        .subscribe((data: any)=>{
            if(data != null){
                this.trackerInstallationReviews = data['object']['records'];
                this.totalRows = data['object']['totalRecords'];
    ...
    delete(deleteType:number, object : TrackerInstallationReview) {
    ...
    confirmDelete(id:number) {
        this.trackerInstallationReviewService.disable(id).subscribe((data: any)=>{
            this.list();
    ...
    confirmDeleteSelected() {
		...
		for(let i = 0 ; i < this.selectedTrackerInstallationReviews.length; i++)
		{
            const ptt = this.trackerInstallationReviewService.disable(this.selectedTrackerInstallationReviews[i].id).pipe
			(
				catchError((error) =>
				{
		...
		forkJoin(peticiones).subscribe((respuestas: any[]) =>
		{
            ...
            this.selectedTrackerInstallationReviews = [];
            this.list();
		},
		err =>
		{
