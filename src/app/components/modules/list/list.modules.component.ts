...
export class ListModulesComponent implements OnInit, OnDestroy
{
	...
	modules: any[]; //coleccion de la lista
    module: any; //elemento individual para elminiar
    ...
    list()
	{
		...
		this.moduleService.getAll(this.limit,this.page,this.sort).subscribe((data: any)=>
		{
			if(data != null)
            {
				this.modules = data['object']['records'];
	...
	{
        this.miscService.startRequest();
		this.moduleService.getFilter(texto,this.limit, this.page,this.sort).subscribe((data: any)=>
		{
			if(data != null)
			{
				this.modules = data.object.records;
    ...
    {
        this.moduleService.disable(id).subscribe((data: any)=>
        {
            this.list();
    ...
    var peticiones: any[] = [];
        ...
        {
            const ptt = this.moduleService.disable(this.selectedElements[i].id).pipe
            (
            catchError((error) =>
            {
