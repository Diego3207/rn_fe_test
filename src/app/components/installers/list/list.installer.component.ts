...
@Component({
    templateUrl: './list.installer.component.html'
})
export class ListInstallerComponent implements OnInit, OnDestroy {
    ...
    list()
    {
        this.installerService.getAll(this.limit,this.page,this.sort)
        .subscribe((data: any)=>{
            if(data != null)
            {
                this.installers = data['object']['records'];
    ...
    filter(texto: any)
    {
        this.installerService.getFilter(texto,this.limit, this.page,this.sort) // le sumo +1 ya que en sails le resto uno a la pagina (en sails quitare ese -1 )
        .subscribe((data: any)=>{
            if(data != null){
                this.installers = data['object']['records'];
                this.totalRows = data['object']['totalRecords'];
    ...
    delete(deleteType:number, object : Installer) {
    ...
    confirmDelete(id:number) {
        this.installerService.disable(id).subscribe((data: any)=>{
            this.list();
    ...
    confirmDeleteSelected() {
		...
		for(let i = 0 ; i < this.selectedInstallers.length; i++)
		{
            const ptt = this.installerService.disable(this.selectedInstallers[i].id).pipe
			(
				catchError((error) =>
				{
