...
export class ListAssociationComponent implements OnInit, OnDestroy {
    ...
    list()
    {
        this.associationService.getAll(this.limit,this.page,this.sort)
        .subscribe((data: any)=>{
            if(data != null)
            {
                console.log(data);
                this.associations = data['object']['records'];
    ...
    filter(texto: any)
    {
        this.associationService.getFilter(texto,this.limit, this.page,this.sort) // le sumo +1 ya que en sails le resto uno a la pagina (en sails quitare ese -1 )
        .subscribe((data: any)=>{
            if(data != null){
                this.associations = data['object']['records'];
                this.totalRows = data['object']['totalRecords'];
    ...
    delete(deleteType:number, object : Association) {
    ...
    confirmDelete(id:number) {
        this.associationService.disable(id).subscribe((data: any)=>{
            this.list();
    ...
    confirmDeleteSelected() {
		...
		{
            const ptt = this.associationService.disable(this.selectedassociations[i].id).pipe
			(
				catchError((error) =>
				{
