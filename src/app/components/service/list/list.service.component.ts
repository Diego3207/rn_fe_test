...
export class ListServiceComponent implements OnInit, OnDestroy {
    ...
    constructor(private miscService:MiscService,private cdref:ChangeDetectorRef,private serviceService: ServiceService, private messageService: MessageService, private confirmationService: ConfirmationService)
	{
    ...
    list(){
        this.serviceService.getAll(this.limit,this.page,this.sort)
        .subscribe((data: any)=>{
            if(data != null)
            {
                this.services = data['object']['records'];
    ...
    filter(texto: any){
        this.serviceService.getFilter(texto,this.limit, this.page,this.sort) // le sumo +1 ya que en sails le resto uno a la pagina (en sails quitare ese -1 )
            .subscribe((data: any)=>{
                if(data != null){
                    this.services = data['object']['records'];
                    this.totalRows = data['object']['totalRecords'];
    ...
    delete(deleteType:number, object : Service) {
    ...
    confirmDelete(id:number) {
        this.serviceService.disable(id).subscribe((data: any)=>{
            this.list();
    ...
    confirmDeleteSelected() {
		...
		{
            const ptt = this.serviceService.disable(this.selectedElements[i].id).pipe
			(
				catchError((error) =>
				{
